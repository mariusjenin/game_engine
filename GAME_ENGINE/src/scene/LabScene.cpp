...
LabScene::LabScene(const std::string &vertex_shader_path, const std::string &fragment_shader_path, float mult_physics) : Scene(
    ...
    cube->get_trsf()->set_translation({-10,20,15});
...
//    m_physics_system.add_collider(rbv_cube2);
   ...
   m_cameras.push_back(camera_node);
    ...
    //DOORS
    DoorElement* door_1 = new DoorElement(m_shaders, m_root, OBB_TYPE);
    door_1->add_behavior(mult_physics);
    door_1->get_trsf()->set_translation({0,11,0});
    m_physics_system->add_collider(door_1->get_right());
    m_physics_system->add_collider(door_1->get_left());
    
    //OUT OF SCENE
    DoorElement* door_2 = new DoorElement(m_shaders, m_root, OBB_TYPE);
    door_2->add_behavior(mult_physics);
    door_2->get_trsf()->set_translation({-scale*9.8,11,0});
    m_physics_system->add_collider(door_2->get_right());
    m_physics_system->add_collider(door_2->get_left());
    

    //DOORWALLS
    glm::vec3 door_pos = door_1->get_trsf()->get_translation();

    auto* wall_left = new NodeGameSG(m_shaders, m_root,OBB_TYPE);
    ...
    wall_left->set_material(new MaterialColor(m_shaders, {0.79, 0.3, 0.3}, 50));
    auto * wall_rbv = new RigidBodyVolume(wall_left);
    wall_rbv->add_behavior(new MovementBehavior(false,false,mult_physics,0));
    m_physics_system->add_collider(wall_rbv);
    
    auto* wall_right = new NodeGameSG(m_shaders, m_root,OBB_TYPE);
    ...
    wall_right->set_material(new MaterialColor(m_shaders, {0.79, 0.3, 0.3}, 50));
    ...
    //BTN
    ButtonElement* button_1 = new ButtonElement(m_shaders, m_root, OBB_TYPE);
    ...
    m_physics_system->add_collider(button_1->get_top());
    button_1->link_door(door_1);
    button_1->add_behavior(mult_physics);

    door_pos = door_2->get_trsf()->get_translation();
    ButtonElement* button_2 = new ButtonElement(m_shaders, m_root, OBB_TYPE);
    ...
    m_physics_system->add_collider(button_2->get_top());
    button_2->link_door(door_2);
    button_2->add_behavior(mult_physics);    
    
    //WARP (change scene trigger)
    auto* warp = new NodeGameSG(m_shaders, m_root,OBB_TYPE);
    ...
    warp->set_material(new MaterialTexture(m_shaders, id_warp_texture));
    RigidBodyVolume* warp_rbv = new RigidBodyVolume(warp);
